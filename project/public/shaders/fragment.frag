...
void main() {
    ...
    // Wave distortion
    float waveX = sin(uv.y * ((uTime/1.0)*1.0) + uTime) * 0.0001;
    float waveY = cos(uv.x * ((uTime/1.0)*1.0) + uTime) * 0.0001;
    vec2 waveOffset = vec2(waveX, waveY);

    // Chromatic aberration
    float aberrationAmount = 0.002;
    vec2 redOffset =  uv + waveOffset + vec2(aberrationAmount, waveY);
    vec2 greenOffset = uv + waveOffset  + vec2(0.0,  0.0);
    vec2 blueOffset = uv + waveOffset - vec2(waveX, aberrationAmount);
