...
const webpackConfig =
	env.RUN_PROJECT == true ? require('../config/webpack.config.dev') : require('../config/webpack.config.headless');
...
(async () => {
	...
	const URL_FXLENS = `http://localhost:${env.PORT_PROJECT}`;
	...
	server.startCallback(() => {
		...
		const l =
			env.RUN_PROJECT === true
				...
				: `${logger.success('[project] your project might be running on, ')} ${logger.url(
						URL_PROJECT
				  )} ${logger.success("but this is user specified so we don't really know")}`;
		console.log(l);
		console.log();
