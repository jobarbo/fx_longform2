...
;(async () => {
  ...
  let oscBridgeProcess = null
  if (env.RUN_OSC_BRIDGE === "1" || env.RUN_OSC_BRIDGE === "true") {
    const bridgePath = path.join(__dirname, "osc-bridge.js")
    oscBridgeProcess = spawn(process.execPath, [bridgePath], {
      cwd: path.join(__dirname, "..", ".."),
      stdio: "inherit",
      env: process.env,
    })
    oscBridgeProcess.on("exit", (code) => {
      if (code != null && code !== 0) {
        console.log(logger.error(`[osc-bridge] exited with code ${code}`))
      }
    })
  }
